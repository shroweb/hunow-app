  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const vouchersResponse = await fetchBusinessVouchers(token).catch(() => []);
        setData(response);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: NAV }}>
      <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingTop: 24, paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ backgroundColor: "rgba(255,255,255,0.07)", borderRadius: 20, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: "rgba(255,255,255,0.08)" }}>
          <Text style={{ color: "rgba(255,255,255,0.46)", fontSize: 13, lineHeight: 20, marginBottom: 14 }}>
            Create one-time giveaway vouchers for your venue. Members claim them with a code, then staff redeem the voucher QR once.
          </Text>
          <View style={{ marginBottom: 12 }}>
            <FieldLabel light>Voucher Title</FieldLabel>
            <Input dark value={voucherDraft.title} onChangeText={(value) => setVoucherDraft((current) => ({ ...current, title: value }))} placeholder="e.g. Free meal for two" />
          </View>
          <View style={{ marginBottom: 12 }}>
            <FieldLabel light>Voucher Code</FieldLabel>
            <Input dark value={voucherDraft.code} onChangeText={(value) => setVoucherDraft((current) => ({ ...current, code: value.toUpperCase() }))} placeholder="e.g. HUNOWMEAL" autoCapitalize="characters" />
          </View>
          <View style={{ marginBottom: 12 }}>
            <FieldLabel light>Description</FieldLabel>
            <Input dark value={voucherDraft.description} onChangeText={(value) => setVoucherDraft((current) => ({ ...current, description: value }))} placeholder="What the member gets" multiline style={{ minHeight: 88, textAlignVertical: "top" }} />
          </View>
          <View style={{ marginBottom: 14 }}>
            <FieldLabel light>Expiry</FieldLabel>
            <Input dark value={voucherDraft.expires_at} onChangeText={(value) => setVoucherDraft((current) => ({ ...current, expires_at: value }))} placeholder="2026-04-30T23:59" autoCapitalize="none" />
          </View>
          <TouchableOpacity
            onPress={handleCreateVoucher}
            disabled={voucherSaving || !voucherDraft.title.trim() || !voucherDraft.code.trim()}
            style={{ backgroundColor: voucherDraft.title.trim() && voucherDraft.code.trim() ? YELLOW : "rgba(251,201,0,0.28)", borderRadius: 16, paddingVertical: 15, alignItems: "center", marginBottom: vouchers.length ? 14 : 0 }}
          >
